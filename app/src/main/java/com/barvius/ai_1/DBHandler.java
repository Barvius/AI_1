...
public class DBHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE symptom (" +
                ...
                "UNIQUE(hash)"+
                ")");
        db.execSQL("CREATE TABLE diagnose (" +
                "id INTEGER PRIMARY KEY," +
                "name TEXT NOT NULL," +
                "hash TEXT NOT NULL," +
                "UNIQUE(hash)"+
                ")");
        db.execSQL("CREATE TABLE symptom_diagnose (" +
    ...
    public Diagnose selectDiagnoseById(long id){
        ...
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery("SELECT symptom_diagnose.diagnose, diagnose.name, symptom_diagnose.symptom, symptom.name " +
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                if(tmpDiagnose != null){
                    tmpDiagnose.addSymptom(new Symptom(cursor.getLong(2),cursor.getString(3)));
                }
            } while (cursor.moveToNext());
        }
        if (tmpDiagnose == null){
    ...
    public static void init(Context context){
        instance = new DBHandler(context,"diagnostic",null,1);
    }
